...
import cm.trixobase.telephone.systeme.database.MySQLiteOpenHelper;
...
public abstract class DomainObject<T> extends MySQLiteOpenHelper {
    ...
    protected long save() {
        try {
            if (getId() != -1) {
                ...
                Log.e(Telephone.Log, "Update Succes in " + getTableName() + " id=" + getId());
            } else {
                id = instanceDatabase.insert(getTableName(), null, data);
                data.put(AttributeName.Id, id);
                Log.e(Telephone.Log, "Insert Succes in " + getTableName() + " :  id=" + getId());
            }
        } catch (SQLiteException ex) {
            Log.e(Telephone.Log, "Exception SQL lors de la sauvegarde - " + ex);
        } catch (Exception e) {
            Log.e(Telephone.Log, "Insert Error in "  + getTableName() + " : with id=" + getId() + "=> " + e);
        }
        return id;
    }
    ...
    protected long delete(long instanceId) {
        try {
            ...
            Log.e(Telephone.Log, "Delete Succes in " + getTableName() + " with id=" + instanceId);
            return 0;
        } catch (SQLiteException ex) {
            Log.e(Telephone.Log, "Exception SQL lors de la suppression (" + getTableName() + "): " + ex);
        } catch (Exception e) {
            Log.e(Telephone.Log, "Delete Error (" + getTableName() + ") - " + getTableName() + " " + e);
        }
        return -1;
    }
